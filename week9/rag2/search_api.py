...
# Extract config values at file start
EMBED_MODEL   = cfg.model.embed_model
MIN_SCORE     = cfg.search.min_score
MIN_FTS_SCORE = cfg.search.min_score_fts

# Path variables
CHUNKS_FILE      = get_rag2_config_path("proc") / cfg.files.chunks_file
FAISS_INDEX_PATH = get_rag2_config_path("indexed") / cfg.files.faiss_index
SIDE_CAR_PATH    = get_rag2_config_path("indexed") / cfg.files.side_car
MANIFEST_PATH    = get_rag2_config_path("indexed") / cfg.files.manifest
...
class KeywordSearchRequest(BaseModel):
    ...
    k:         int = Field(5, ge=1, le=50, description="Top-k results to return")
    min_score: float = Field(
        default=MIN_FTS_SCORE, ge=0, le=1,
...
class HybridSearchRequest(BaseModel):
    ...
    k:            int   = Field(5, ge=1, le=50, description="Top-k results to return")
    min_score:    float = Field(default=MIN_SCORE, ge=0, le=1, description="Minimum semantic score")
...
async def lifespan(app: FastAPI):
    ...
    try:
        test_conn = sqlite3.connect(CHUNKS_FILE)
    ...
    try:
        bm25_conn = sqlite3.connect(CHUNKS_FILE)
