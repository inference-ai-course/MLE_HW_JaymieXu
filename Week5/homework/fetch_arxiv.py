...
def _load_doc_titles() -> dict[str, str]:
    ...
    if cfg.DOCS_OUT.exists():
        conn   = sqlite3.connect(cfg.DOCS_OUT)
        cursor = conn.execute("SELECT doc_id, title FROM documents")
        m      = {row[0]: row[1] or "" for row in cursor.fetchall()}
        conn.close()
...
def _iter_chunks() -> Iterator[tuple[str, dict[str, Any]]]:
      ...
      for row in cursor.fetchall():
          # May not even need this check now
          txt = row[0].strip()
          if not txt:
              continue
...
def run_faiss() -> None:
    ...
    sidecar_conn = sqlite3.connect(cfg.SIDE_CAR_PATH)
    ...
    def flush(flush_buf_txt: list[str], flush_buf_meta: list[dict]):
        ...
        for m in flush_buf_meta:
            ...
            sidecar_conn.execute('''
                                 ...
                                 VALUES (?, ?, ?, ?)
                                 ''',
                                 (m["chunk_id"], m["doc_id"], m["page"], m["title"]))
...
def run_extract(limit: int | None = None):
    ...
    try:
        ...
        conn.close()  # ADD: Close database connection
        if page_f:
            page_f.close()
...
def run_chunk(limit_docs: int | None = None):
    ...
    docs_conn   = sqlite3.connect(cfg.DOCS_OUT)
    
    try:
        doc_cursor = docs_conn.execute("SELECT doc_id, title, text FROM documents WHERE doc_id NOT IN ({})".format(','.join('?' * len(already))), list(already))
        ...
        chunks_conn.commit()  # ADD: Commit all changes
